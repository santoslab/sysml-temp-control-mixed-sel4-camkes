...
// send fanAck: Out EventDataPort TempControlMixedSeL4CAmKES__FanAck
Unit tc_TempControlMixedSeL4CAmKES_Fan_fp_f_seL4Nix_fanAck_Send(
  STACK_FRAME
  art_DataContent d) {
  DeclNewStackFrame(caller, "sb_Fan.c", "", "tc_TempControlMixedSeL4CAmKES_Fan_fp_f_seL4Nix_fanAck_Send", 0);

  sb_fanAck_enqueue(d);
}

// is_empty fanCmd: In EventDataPort
B tc_TempControlMixedSeL4CAmKES_Fan_fp_f_seL4Nix_fanCmd_IsEmpty(STACK_FRAME_ONLY) {
  return sb_fanCmd_is_empty();
}

// receive fanCmd: In EventDataPort union_art_DataContent
Unit tc_TempControlMixedSeL4CAmKES_Fan_fp_f_seL4Nix_fanCmd_Receive(
  STACK_FRAME
  Option_8E9F45 result) {
  DeclNewStackFrame(caller, "sb_Fan.c", "", "tc_TempControlMixedSeL4CAmKES_Fan_fp_f_seL4Nix_fanCmd_Receive", 0);
...
void pre_init(void) {
  DeclNewStackFrame(NULL, "sb_Fan.c", "", "pre_init", 0);

  printf("Entering pre-init of Fan_fp_f\n");
  ...
  tc_TempControlMixedSeL4CAmKES_Fan_fp_f_adapter_initialiseArchitecture(SF_LAST);

  // call the component's initialise entrypoint
  tc_TempControlMixedSeL4CAmKES_Fan_fp_f_adapter_initialiseEntryPoint(SF_LAST);

  printf("Leaving pre-init of Fan_fp_f\n");
}

void post_init(void) {
  DeclNewStackFrame(NULL, "sb_Fan.c", "", "post_init", 0);
 ...
 ************************************************************************/
int run(void) {
  DeclNewStackFrame(NULL, "sb_Fan.c", "", "run", 0);

  MUTEXOP(sb_dispatch_sem_wait())
  for(;;) {
    MUTEXOP(sb_dispatch_sem_wait())
    // call the component's compute entrypoint
    tc_TempControlMixedSeL4CAmKES_Fan_fp_f_adapter_computeEntryPoint(SF_LAST);
  }
  return 0;
}
