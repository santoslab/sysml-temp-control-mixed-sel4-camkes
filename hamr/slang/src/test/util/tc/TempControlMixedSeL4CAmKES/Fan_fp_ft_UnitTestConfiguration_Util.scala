...
object Fan_fp_ft_UnitTestConfiguration_Util {
  ...
  type DefaultComputeProfile = Fan_fp_ft_Profile_P

  def defaultComputeConfig: Fan_fp_ft_Compute_UnitTestConfiguration = {
    return (Fan_fp_ft_Compute_UnitTestConfiguration (
      verbose = F,
      ...
      genReplay = (c: Container, testName: String, r: GumboXResult.Type) => Some(
       st"""Replay Unit Test:
            ...
            |    val testVector = tc.JSON.toTempControlMixedSeL4CAmKESFan_fp_ft_PreState_Container_P(json).left
...
@record class Fan_fp_ft_Compute_UnitTestConfiguration (
  var verbose: B,
  var name: String,
  ...
  var profile: Fan_fp_ft_Profile_P_Trait,
  var genReplay: (Container, String, GumboXResult.Type) => Option[String])
  extends UnitTestConfigurationBatch with Fan_fp_ft_GumboX_TestHarness {

  override def test(c: Container): GumboXResult.Type = {
    return testComputeCBV(c.asInstanceOf[Fan_fp_ft_PreState_Container])
  }
}
