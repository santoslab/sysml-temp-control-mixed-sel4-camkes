...
object Schedulers {
  ...
  val threadNickNames: Map[String, Art.BridgeId] = Map(
    ISZ(
      Arch.TempControlSystem_Instance_tsp_ts.name ~> Arch.TempControlSystem_Instance_tsp_ts.id,
      Arch.TempControlSystem_Instance_tcp_tc.name ~> Arch.TempControlSystem_Instance_tcp_tc.id,
      Arch.TempControlSystem_Instance_fp_f.name ~> Arch.TempControlSystem_Instance_fp_f.id)
  )
  ...
  val TempControlSystem_Instance_tsp_ts_timingProperties: ThreadTimingProperties = ThreadTimingProperties(
    computeExecutionTime = None(),
    domain = None())

  val TempControlSystem_Instance_tcp_tc_timingProperties: ThreadTimingProperties = ThreadTimingProperties(
    computeExecutionTime = None(),
    domain = None())

  val TempControlSystem_Instance_fp_f_timingProperties: ThreadTimingProperties = ThreadTimingProperties(
    computeExecutionTime = None(),
    domain = None())
  ...
  val defaultDomainToBridgeIdMap: ISZ[Art.BridgeId] = ISZ(
    /* domain 0 */ Arch.TempControlSystem_Instance_tsp_ts.id,
    /* domain 1 */ Arch.TempControlSystem_Instance_tcp_tc.id,
    /* domain 2 */ Arch.TempControlSystem_Instance_fp_f.id
  )
