...
object Schedulers {
  ...
  val threadNickNames: Map[String, Art.BridgeId] = Map(
    ISZ(
      Arch.TempControlSystem_Instance_tsp_tst.name ~> Arch.TempControlSystem_Instance_tsp_tst.id,
      Arch.TempControlSystem_Instance_tcp_tct.name ~> Arch.TempControlSystem_Instance_tcp_tct.id,
      Arch.TempControlSystem_Instance_fp_ft.name ~> Arch.TempControlSystem_Instance_fp_ft.id)
  )
  ...
  val TempControlSystem_Instance_proc_timingProperties: ProcessorTimingProperties = ProcessorTimingProperties(
    clockPeriod = Some(1),
    framePeriod = Some(1000),
    maxDomain = None(),
    slotTime = None())

  val TempControlSystem_Instance_tsp_tst_timingProperties: ThreadTimingProperties = ThreadTimingProperties(
    computeExecutionTime = None(),
    domain = Some(3))

  val TempControlSystem_Instance_tcp_tct_timingProperties: ThreadTimingProperties = ThreadTimingProperties(
    computeExecutionTime = None(),
    domain = Some(2))

  val TempControlSystem_Instance_fp_ft_timingProperties: ThreadTimingProperties = ThreadTimingProperties(
    computeExecutionTime = None(),
    domain = Some(4))
