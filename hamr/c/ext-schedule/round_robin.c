...
void tc_ScheduleProviderI_getRoundRobinOrder(STACK_FRAME IS_FDDCB6 result) {
  ...
  if(tc_Schedulers_roundRobinSchedule) {
    ...
    // example schedule
    int i = 0;
    IS_FDDCB6_up(result, i++, (art_Art_BridgeId) tc_Arch_TempControlSystem_Instance_tsp_tst(SF_LAST)->id);
    IS_FDDCB6_up(result, i++, (art_Art_BridgeId) tc_Arch_TempControlSystem_Instance_tcp_tct(SF_LAST)->id);
    IS_FDDCB6_up(result, i++, (art_Art_BridgeId) tc_Arch_TempControlSystem_Instance_fp_ft(SF_LAST)->id);
