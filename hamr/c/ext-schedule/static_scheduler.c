...
void tc_ScheduleProviderI_getStaticSchedule(STACK_FRAME art_scheduling_static_Schedule_DScheduleSpec result){
  ...
  if(tc_Schedulers_defaultStaticSchedule) {
    ...
    int i = 0;
    fillInSlot(&slotSequence, i++, tc_Arch_TempControlSystem_Instance_tsp_tst(SF_LAST)->id, length);
    fillInSlot(&slotSequence, i++, tc_Arch_TempControlSystem_Instance_tcp_tct(SF_LAST)->id, length);
    fillInSlot(&slotSequence, i++, tc_Arch_TempControlSystem_Instance_fp_ft(SF_LAST)->id, length);
    slotSequence.size = i;
